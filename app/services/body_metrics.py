...
class bodyMetrics:
    def __init__(self, weight, height, age, impedance, sex):
        ...
        if self.height > 220:
            raise ValueError(f"Height too high: {self.height} cm (max 220)")
        if weight < 10 or weight > 200:
            raise ValueError(f"Weight out of range: {weight} kg (10-200)")
        if age > 99:
            raise ValueError(f"Age too high: {age} (max 99)")
        if impedance > 3000:
            raise ValueError(f"Impedance too high: {impedance} Ohm (max 3000)")
