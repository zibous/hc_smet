// dateselector.js

import { setCurrentRange, setCompareActive } from "./state.js";

let stylesInjected = false;

/* ----------------------------------------------------
   STYLES
---------------------------------------------------- */
function injectStyles() {
  if (stylesInjected) return;

  const style = document.createElement("style");
  style.innerHTML = `
    .header {
      position: relative !important;
      z-index: 1000 !important;
    }

    .period-container {
      position: relative;
      display: inline-flex;
      align-items: center;
    }

    .period-btn {
      padding: 0.8rem 1.4rem;
      border-radius: 999px;
      border: 1px solid var(--border, rgba(255,255,255,0.15));
      background: var(--card-bg, rgba(255,255,255,0.08));
      color: var(--text-main, #ffffff);
      cursor: pointer;
      font-size: 1.35rem;
      font-weight: 500;
      backdrop-filter: blur(20px);
      -webkit-backdrop-filter: blur(20px);
    }

    .period-btn::after {
      content: " ▾";
      opacity: 0.7;
    }

    .period-dropdown {
      position: absolute !important;
      top: calc(100% + 0.5rem);
      right: 0;
      min-width: 240px;
      background: var(--card-bg, #ffffff);
      border-radius: 1.2rem;
      border: 1px solid var(--border, #e2e8f0);
      padding: 0.5rem;
      box-shadow: 0 10px 30px rgba(0,0,0,0.25);
      backdrop-filter: blur(25px);
      -webkit-backdrop-filter: blur(25px);
      z-index: 99999 !important;

      /* Begrenzt die Höhe des Dropdowns und fügt einen Scrollbalken hinzu */
      max-height: 400px;
      overflow-y: auto;
    }

    .period-dropdown.hidden {
      display: none;
    }

    .period-item {
      padding: 0.8rem 1.2rem;
      border-radius: 0.8rem;
      font-size: 1.35rem;
      color: var(--text-main, #2d3748);
      cursor: pointer;
    }

    .period-item:hover {
      background: rgba(128, 128, 128, 0.08);
    }

    .period-item.custom-toggle {
      border-top: 1px solid var(--border, #edf2f7);
      margin-top: 0.4rem;
      padding-top: 0.8rem;
      color: var(--primary, #3182ce);
      font-weight: 600;
    }

    .inline-range-box {
      padding: 0.8rem;
      border-top: 1px dashed var(--border, #edf2f7);
      margin-top: 0.4rem;
      display: flex;
      flex-direction: column;
      gap: 0.6rem;
    }

    .inline-range-box.hidden {
      display: none;
    }

    .inline-field {
      display: flex;
      justify-content: space-between;
      align-items: center;
      gap: 1rem;
    }

    .inline-field label {
      font-size: 1.2rem;
      font-weight: 600;
      color: var(--text-muted, #718096);
    }

    .inline-range-box input[type="date"] {
      padding: 0.5rem;
      border-radius: 0.6rem;
      border: 1px solid var(--border, #cbd5e1);
      background-color: var(--bg-color, #f7fafc) !important;
      color: var(--text-main, #2d3748) !important;
      font-size: 1.25rem;
      outline: none;
      font-family: inherit;
    }

    [data-theme="dark"] .inline-range-box input[type="date"] {
      background-color: #0f172a !important;
      color: #f8fafc !important;
      border-color: #334155 !important;
      color-scheme: dark !important;
    }

    [data-theme="dark"] .inline-range-box input[type="date"]::-webkit-calendar-picker-indicator {
      filter: invert(1) !important;
      cursor: pointer;
      opacity: 0.8;
    }

    .inline-action-btn {
      width: 100%;
      padding: 0.7rem;
      border-radius: 0.6rem;
      border: none;
      background: var(--primary, #3182ce);
      color: #ffffff;
      font-size: 1.3rem;
      font-weight: 600;
      cursor: pointer;
    }

    .toggle { display: flex; align-items: center; gap: 0.7rem; cursor: pointer; font-size: 1.3rem; }
    .toggle input { display: none; }
    .slider { position: relative; width: 3.6rem; height: 2rem; border-radius: 999px; background: #cbd5e1; transition: .2s ease; }
    .slider::before { content: ""; position: absolute; top: 0.2rem; left: 0.2rem; width: 1.6rem; height: 1.6rem; border-radius: 50%; background: #ffffff; transition: .2s ease; }
    .toggle input:checked + .slider { background: var(--primary, #3182ce); }
    .toggle input:checked + .slider::before { transform: translateX(1.6rem); }
  `;
  document.head.appendChild(style);
  stylesInjected = true;
}

/* ----------------------------------------------------
   DATUMS-BERECHNUNGEN
---------------------------------------------------- */
function getPeriodRange(periodKey) {
  const now = new Date();
  let fromDate = new Date();
  let toDate = new Date();

  fromDate.setHours(0, 0, 0, 0);
  toDate.setHours(23, 59, 59, 999);

  // 🔧 PARST HISTORISCHE JAHRE (z.B. "jahr_2014" → 2014)
  if (periodKey.startsWith("jahr_")) {
    const targetYear = parseInt(periodKey.split("_")[1]);
    fromDate.setFullYear(targetYear, 0, 1);
    toDate.setFullYear(targetYear, 11, 31);
  } else {
    // Reguläre Weiche
    switch (periodKey) {
      case "heute": break;
      case "gestern":
        fromDate.setDate(now.getDate() - 1);
        toDate.setDate(now.getDate() - 1);
        break;
      case "woche": {
        const day = now.getDay();
        const diff = now.getDate() - day + (day === 0 ? -6 : 1);
        fromDate.setDate(diff);
        break;
      }
      case "7tage": fromDate.setDate(now.getDate() - 6); break;
      case "30tage": fromDate.setDate(now.getDate() - 29); break;
      case "monat": fromDate.setDate(1); break;
      case "jahr": fromDate.setMonth(0, 1); break;
    }
  }

  const formatISO = (d) => {
    const pad = (num) => String(num).padStart(2, "0");
    return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`;
  };

  return { from: formatISO(fromDate), to: formatISO(toDate) };
}

/* ----------------------------------------------------
   RENDER PIPELINE
---------------------------------------------------- */
export function renderPeriodStrip(onRangeChanged) {
  injectStyles();

  const container = document.querySelector(".controls");
  if (!container) return;

  const savedRangeStr = localStorage.getItem("dash_range");
  const savedRange = savedRangeStr ? JSON.parse(savedRangeStr) : null;
  const savedCompare = localStorage.getItem("dash_compare") !== "false";

  let selectedLabel = localStorage.getItem("dash_btn_label") || "Heute";
  let activeRange = savedRange || getPeriodRange("heute");

  // 🔧 DYNAMISCHE JAHRES-SCHLEIFE GENERIEREN:
  // Wir zählen vom aktuellen Jahr rückwärts bis 2013 herab.
  const currentYear = new Date().getFullYear();
  let yearsHtml = "";
  for (let year = currentYear; year >= 2013; year--) {
    yearsHtml += `<div class="period-item" data-key="jahr_${year}">Jahr ${year}</div>`;
  }

  container.innerHTML = `
    <div class="period-container">
      <button class="period-btn" id="periodBtn">${selectedLabel}</button>

      <div class="period-dropdown hidden" id="periodDropdown">
        <div class="period-item" data-key="heute">Heute</div>
        <div class="period-item" data-key="gestern">Gestern</div>
        <div class="period-item" data-key="woche">Diese Woche</div>
        <div class="period-item" data-key="7tage">Letzte 7 Tage</div>
        <div class="period-item" data-key="30tage">Letzte 30 Tage</div>
        <div class="period-item" data-key="monat">Dieser Monat</div>

        <!-- INJEKTION DER DYNAMISCHEN HISTORIE (2013 - Heute) -->
        ${yearsHtml}

        <div class="period-item custom-toggle" id="customToggleItem">Benutzerdefiniert…</div>

        <div class="inline-range-box hidden" id="inlineRangeBox">
          <div class="inline-field">
            <label>Von</label>
            <input type="date" id="inlineFromDate">
          </div>
          <div class="inline-field">
            <label>Bis</label>
            <input type="date" id="inlineToDate">
          </div>
          <button class="inline-action-btn" id="inlineApplyBtn">Anwenden</button>
        </div>
      </div>
    </div>

    <label class="toggle">
      <input type="checkbox" id="compareToggle" ${savedCompare ? "checked" : ""}>
      <span class="slider"></span>
      <span class="label">Vergleich</span>
    </label>
  `;

  const periodBtn = document.getElementById("periodBtn");
  const periodDropdown = document.getElementById("periodDropdown");
  const customToggleItem = document.getElementById("customToggleItem");
  const inlineRangeBox = document.getElementById("inlineRangeBox");
  const compareToggle = document.getElementById("compareToggle");

  const inlineFromInput = document.getElementById("inlineFromDate");
  const inlineToInput = document.getElementById("inlineToDate");
  const inlineApplyBtn = document.getElementById("inlineApplyBtn");

  const triggerUpdate = () => {
    setCurrentRange(activeRange, selectedLabel);
    setCompareActive(compareToggle.checked);

    onRangeChanged({
      from: activeRange.from,
      to: activeRange.to,
      compare: compareToggle.checked
    });
  };

  periodBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    periodDropdown.classList.toggle("hidden");
  });

  document.addEventListener("click", () => {
    periodDropdown.classList.add("hidden");
  });

  periodDropdown.addEventListener("click", (e) => {
    e.stopPropagation();
  });

  // Liest alle period-items (inklusive der generierten Jahre) dynamisch aus
  periodDropdown.querySelectorAll(".period-item:not(.custom-toggle)").forEach(item => {
    item.addEventListener("click", () => {
      selectedLabel = item.textContent;
      periodBtn.textContent = selectedLabel;
      activeRange = getPeriodRange(item.getAttribute("data-key"));
      inlineRangeBox.classList.add("hidden");
      periodDropdown.classList.add("hidden");
      triggerUpdate();
    });
  });

  customToggleItem.addEventListener("click", () => {
    if (!inlineFromInput.value) {
      const todayISO = new Date().toISOString().split("T");
      inlineFromInput.value = todayISO;
      inlineToInput.value = todayISO;
    }
    inlineRangeBox.classList.toggle("hidden");
  });

  inlineApplyBtn.addEventListener("click", () => {
    const fromVal = inlineFromInput.value;
    const toVal = inlineToInput.value;

    if (!fromVal || !toVal) return;

    activeRange = {
      from: `${fromVal}T00:00:00`,
      to: `${toVal}T23:59:59`
    };

    selectedLabel = `${fromVal.split("-").reverse().slice(0,2).join(".")} - ${toVal.split("-").reverse().slice(0,2).join(".")}`;
    periodBtn.textContent = selectedLabel;

    periodDropdown.classList.add("hidden");
    triggerUpdate();
  });

  compareToggle.addEventListener("change", triggerUpdate);

  triggerUpdate();
}
