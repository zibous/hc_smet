...
print("FILES:", list(DATA_DIR.glob("*.db")) + list(DATA_DIR.glob("*.sqlite")))
...
def build_daily(rows):
    ...
    for (sensor, day), values in grouped.items():
...
def save_all(yearly, clusters, daily):
...
def main():
    ...
    rows = load_hourly_values()

    print("Rows:", len(rows))

    init_database()

    yearly = build_yearly(rows)
    clusters = build_clusters(rows)
    daily = build_daily(rows)

    save_all(yearly, clusters, daily)
